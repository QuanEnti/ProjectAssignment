...
public void clearCart(int customerId) {
    ...
    try {
        tx.begin();
        int rows = em.createQuery("DELETE FROM CartItem c WHERE c.customer.id = :cid")
                     .setParameter("cid", customerId)
                     .executeUpdate();
        System.out.println("Xóa giỏ hàng: " + rows + " dòng");
        tx.commit();
    } catch (Exception e) {
        if (tx.isActive()) tx.rollback();
        e.printStackTrace();
...
public void deleteByCustomer(Customer customer) {
    ...
    try {
        ...
        // Đảm bảo customer là entity được quản lý
        Customer managedCustomer = em.find(Customer.class, customer.getId());

        int rows = em.createQuery("DELETE FROM CartItem c WHERE c.customer = :customer")
                     .setParameter("customer", managedCustomer)
                     .executeUpdate();

        System.out.println("CartItemDAO | Xóa bằng customer entity: " + rows + " dòng");
    ...
    try {
        ...
        // 💥 Dùng native SQL sẽ chắc chắn xóa được
        int rows = em.createNativeQuery("DELETE FROM CartItem WHERE customerId = ?")
                     .setParameter(1, customerId)
                     .executeUpdate();

        System.out.println("CartItemDAO | Native SQL xóa " + rows + " dòng của customerId=" + customerId);
